...
const Center = () => {
  ...
  return (
    <div className="flex-grow text-white h-screen overflow-y-scroll scrollbar-hide">
      ...
      <div className={`h-10 w-52 rounded-sm bg-[#2e2e2e] text-white absolute right-8 top-[4.2rem] flex-col` + ' ' + `${isActive ? 'hidden' : 'flex'}`}>
        <div className="flex items-center justify-between cursor-pointer px-3 py-2" onClick={signOut}>
          <p className="hover:bg-[#2b2d30]">Sign Out</p>
          <ArrowLeftOnRectangleIcon className="w-5 h-5" />
        </div>
      </div>

      <section className={`flex items-end space-x-7 bg-gradient-to-b to-black ${color} h-80 text-white pl-6 pb-6 border-[.5px] border-gray-600`}>
        <img className="h-40 w-40 shadow-2xl" src={playlist?.images?.[0]?.url} alt="album img" />
        <div>
          <p>Playlist</p>
